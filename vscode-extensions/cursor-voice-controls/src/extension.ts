...
class VoiceControls {
    ...
    private playbackSpeed: number = 1.0; // Normal speed
    private skipInterval: number = 5; // seconds to skip
    
    // Auto-hide functionality
    private autoHideEnabled: boolean = true;
    private hideTimeout: NodeJS.Timeout | null = null;
    private hoverTimeout: number = 2000; // Hide after 2 seconds of no hover
    private isHovering: boolean = false;
    private statusBarHoverDisposable: vscode.Disposable | null = null;
    
    // Hands-free auto-send functionality
    private autoSendEnabled: boolean = true;
    private pauseDuration: number = 4000; // Default 4 seconds (3-5 range)
    ...
    private setupAutoSend(): void {
        ...
        this.pauseDuration = config.get<number>('pauseDuration', 4000); // Default 4 seconds
    ...
    public enableAutoSend(enabled: boolean): void {
     ...
     */
    public async skipBack(): Promise<void> {
        this.cancelHideTimer(); // Show when user interacts
     ...
     */
    public async fastRewind(): Promise<void> {
        if (this.state === VoiceState.Stopped) {
            vscode.window.showWarningMessage('Voice transcription is not active');
            return;
        }
...
let voiceControls: VoiceControls;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    vscode.workspace.onDidChangeConfiguration(e => {
        if (e.affectsConfiguration('cursorVoiceControls.autoHide')) {
            const newAutoHide = config.get<boolean>('autoHide', true);
            voiceControls.enableAutoHide(newAutoHide);
        }
        if (e.affectsConfiguration('cursorVoiceControls.autoHideDelay')) {
            const newDelay = config.get<number>('autoHideDelay', 2000);
    ...
    const toggleAutoHideCommand = vscode.commands.registerCommand('cursorVoiceControls.toggleAutoHide', () => {
        const currentAutoHide = config.get<boolean>('autoHide', true);
    ...
    const toggleAutoSendCommand = vscode.commands.registerCommand('cursorVoiceControls.toggleAutoSend', () => {
        const currentAutoSend = config.get<boolean>('autoSend', true);
