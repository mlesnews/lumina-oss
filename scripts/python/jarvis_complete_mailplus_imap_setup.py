#!/usr/bin/env python3
"""
JARVIS Complete MailPlus IMAP Setup - Full Autonomous Control

JARVIS takes full autonomous control to complete MailPlus IMAP configuration:
1. Enable IMAP port 993 on MailPlus via SSH/DSM API
...
Tags: #JARVIS #OUTLOOK #AUTOMATION #MAILPLUS #FULLAUTO
...
logger = get_logger("JARVISMailPlusSetup")


class JARVISCompleteMailPlusSetup:
    """JARVIS autonomous control for complete MailPlus email setup"""
    ...
    def __init__(self):
        ...
        logger.info("=" * 80)
        logger.info("🤖 JARVIS AUTONOMOUS MAILPLUS EMAIL SETUP")
        logger.info("=" * 80)
        logger.info("")
    ...
    def enable_imap_via_ssh(self) -> bool:
        """Enable IMAP port 993 via SSH for MailPlus"""
        try:
            ...
            # Enable IMAP service via MailPlus
            logger.info("⚙️  Enabling IMAP service on MailPlus...")
            ...
            if "is running" in mailplus_status.lower():
                logger.info("✅ MailPlus is running")
            else:
                logger.warning("⚠️  MailPlus may not be running")
                # Try to start it
                stdin, stdout, stderr = ssh.exec_command("synopkg start MailPlus")
                time.sleep(3)

            # Enable IMAP via MailPlus configuration
            # MailPlus config is typically in /usr/syno/etc/mail/mailserver.conf
            logger.info("📝 Configuring MailPlus IMAP...")

            # Check MailPlus config location
            config_paths = [
                "/usr/syno/etc/mail/mailserver.conf",
                "/var/packages/MailPlus/etc/mailserver.conf"
            ]

            for config_path in config_paths:
                stdin, stdout, stderr = ssh.exec_command(
                    f"test -f {config_path} && echo 'exists' || echo 'not_found'"
                )
                if stdout.read().decode().strip() == "exists":
                    logger.info(f"✅ Found MailPlus config: {config_path}")
                    # Could modify config here if needed
                    break

            # Enable IMAP service via synoservice
            logger.info("   Enabling IMAP service...")
            stdin, stdout, stderr = ssh.exec_command(
                "synoservice --enable pkgctl-MailPlus"
            )
            time.sleep(2)

            # Check if IMAP is enabled in MailPlus settings
            # This may require DSM web interface or API
            logger.info("💡 IMAP configuration may require DSM web interface")
            logger.info("   MailPlus → Settings → Mail Service → Enable IMAP (993)")
    ...
    def enable_imap_via_dsm_api(self) -> bool:
        """Enable IMAP via DSM API for MailPlus"""
        try:
            from nas_azure_vault_integration import NASAzureVaultIntegration
            ...
            # Try to access MailPlus package API
            logger.info("⚙️  Attempting to configure MailPlus IMAP...")

            # MailPlus API endpoints (may vary by DSM version)
            mailplus_url = f"{api.base_url}/webapi/entry.cgi"

            endpoints_to_try = [
                {
                    "api": "SYNO.MailPlus.MailService",
                    "version": "1",
                    "method": "set",
                    "params": {
                        "imap_enable": "true",
            ...
            for endpoint in endpoints_to_try:
                try:
                    ...
                    response = api.session.get(mailplus_url, params=params, timeout=10)
                    data = response.json()
            ...
            logger.info("💡 MailPlus IMAP configuration requires DSM web interface:")
            logger.info("   1. Access DSM: https://<NAS_PRIMARY_IP>:5001")
    ...
    def setup_outlook(self, password: Optional[str]) -> bool:
        """Setup Outlook Classic account for MailPlus"""
        try:
    ...
    def execute_full_setup(self) -> Dict[str, Any]:
        ...
        if self.check_imap_port():
            ...
            if self.enable_imap_via_ssh():
                ...
                # Try DSM API as well
                logger.info("   Attempting DSM API configuration...")
                self.enable_imap_via_dsm_api()
                ...
                logger.info("   DSM → MailPlus → Settings → Mail Service → Enable IMAP (993)")
...
def main():
    try:
        ...
        parser = argparse.ArgumentParser(description="JARVIS Complete MailPlus Email Setup")
        parser.add_argument("--password", type=str, help="NAS Mail Hub password")
        parser.add_argument("--json", action="store_true", help="Output as JSON")

        args = parser.parse_args()

        jarvis = JARVISCompleteMailPlusSetup()
