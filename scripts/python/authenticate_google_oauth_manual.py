"""
Manual Google OAuth Authentication Script

This script provides a manual OAuth flow that doesn't require browser automation.
Useful when browser connectivity issues prevent automatic browser opening.
...
logger = logging.getLogger("GoogleOAuthManual")
...
class ManualGoogleOAuthAuthenticator:
    """Handles manual Google OAuth authentication flow."""
    ...
    def manual_authenticate(self, force: bool = False) -> bool:
        """Execute manual OAuth authentication flow (no browser automation)."""
        ...
        # Manual OAuth flow
        logger.info("\n" + "="*80)
        logger.info("MANUAL GOOGLE OAUTH AUTHENTICATION")
        logger.info("="*80)
        logger.info(f"Credentials file: {self.credentials_file}")
        logger.info(f"Scopes: {', '.join(self.scopes)}")
        logger.info("\n📋 Manual OAuth Flow - No browser automation required")
        logger.info("="*80 + "\n")
        ...
        try:
            ...
            logger.info("\n🔗 STEP 1: Copy this authorization URL and open it in ANY browser:")
            logger.info("   (You can use Chrome, Edge, Firefox, or even Neo if it's working)")
            logger.info("\n" + "="*80)
            logger.info(auth_url)
            logger.info("="*80 + "\n")

            logger.info("📋 STEP 2: Complete the OAuth flow in your browser:")
            logger.info("   1. Sign in with your Google account")
            logger.info("   2. Grant the requested permissions")
            logger.info("   3. You'll be redirected to a page with an authorization code")
            logger.info("   4. Copy the ENTIRE authorization code from the URL or page")
            logger.info("\n💡 The authorization code is the value after 'code=' in the URL")
            logger.info("   Example: https://localhost/?code=4/0A...&scope=...")
            logger.info("   Copy everything after 'code=' up to (but not including) '&'")
            logger.info("\n" + "-"*80)

            # Get authorization code from user
            auth_code = input("\n🔑 STEP 3: Paste the authorization code here and press Enter: ").strip()
            ...
            if 'code=' in auth_code:
                auth_code = auth_code.split('code=')[1].split('&')[0].split(' ')[0]
            ...
            logger.info("\n💡 Troubleshooting:")
            logger.info("   1. Check if authorization code was copied correctly")
            logger.info("   2. Authorization codes expire quickly - use it immediately")
            logger.info("   3. Make sure you completed the OAuth flow in the browser")
            logger.info("   4. Verify OAuth consent screen is configured correctly")
            logger.info("   5. Ensure you're added as a test user (if app is in testing mode)")
            return False

    def build_service(self, service_name: str, version: str = 'v3') -> bool:
        """Build a Google API service."""
        if not self.creds:
            if not self.manual_authenticate():
                return False
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Manual Google OAuth authentication (no browser automation)")
    ...
    # Determine scopes
    scopes_map = {
        'drive': ManualGoogleOAuthAuthenticator.SCOPES['drive'],
        'gmail': ManualGoogleOAuthAuthenticator.SCOPES['gmail'],
        'both': ManualGoogleOAuthAuthenticator.SCOPES['both']
    }
    scopes = scopes_map[args.scopes]

    authenticator = ManualGoogleOAuthAuthenticator(args.project_root, scopes=scopes)
    ...
    if args.test or args.test_drive_only or args.test_gmail_only:
        ...
        # Authentication mode
        success = authenticator.manual_authenticate(force=args.force)
